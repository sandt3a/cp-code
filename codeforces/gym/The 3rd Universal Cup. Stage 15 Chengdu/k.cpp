...
struct network_simplex {
    ...
    int add_edge(int u, int v, Flow lower, Flow upper, Cost cost) {
...
__attribute__((target("avx"), optimize("O3", "unroll-loops")))
std::vector<pair<i64, int>> factor(i64 n) {
    std::vector<i64> p;
    ...
    return std::vector<pair<i64, int>>(mp.begin(), mp.end());
}

__attribute__((target("avx"), optimize("O3", "unroll-loops")))
auto divisor(const std::vector<pair<i64, int>> &d) {
...
void solve()
{
    ...
    vector<i64> vals;
    vector<vector<pair<i64, int>>> ds(n);
    for (int i = 0; i < n; i++) {
        ds[i] = factor(a[i]);
        auto d = divisor(ds[i]);
        vals.insert(vals.end(), d.begin(), d.end());
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        std::function<void(int, int, int, int)> dfs = [&](int x, int y, int cur, int cc) {
            if (cnt >= n) return;
            if (x == sz) {
                if (cur < a[i]) {
                    g.add_edge(id, get(cur), 0, inf, cc - tot);
                    cnt++;
                }
                return;
            }
    ...
    int t = m + 1;
    for (int i = 0; i < n; i++) {
        g.add_edge(s, get(a[i]), 0, 1, 0);
    }
    for (int i = 0; i < m; i++) {
        g.add_edge(i, t, 0, 1, 0);
    }

    g.set_supply(s, inf);
    g.set_supply(t, -inf);
    auto f = g.mincost_flow();
    assert(f == n);
    auto ans = g.get_circulation_cost();
